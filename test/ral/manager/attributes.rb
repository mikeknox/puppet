...
class TestTypeAttributes < Test::Unit::TestCase
  ...
  def test_newattr
    ...
    attr_check(type) do |inst, klass|
      ...
      assert_raise(Puppet::Error, "Did not throw an error when replacing attr") do
  ...
  def test_newattr_and_required_features
    ...
    [nope, maybe, yep].each_with_index do |prov, i|
      ...
      no.each do |a|
        assert_nil(resource.should(a), "Got value for unsupported #{a} in #{prov.name}")
        if Puppet::Util::Log.sendlevel?(:info)
          assert(@logs.find { |l| l.message =~ /not managing attribute #{a}/ and l.level == :info }, "No warning about failed #{a}")
        end
      end
